...
namespace Vampire.RL.Tests
{
    ...
    public class StateEncoderTest : MonoBehaviour
    {
        ...
        public void RunAllTests()
        {
            ...
            allTestsPassed &= TestTeammateDataEncoding();
        ...
        private void SetUp()
        {
            ...
            testGameState = new RLGameState
            {
                ...
                teammates = new TeammateInfo[]
                {
                    new TeammateInfo { position = new Vector2(6f, 7f), velocity = new Vector2(1f, 0f), health = 90f, isDowned = false },
                    new TeammateInfo { position = new Vector2(-3f, 2f), velocity = new Vector2(0f, -2f), health = 50f, isDowned = true }
                },
        ...
        private bool TestStateSize()
        {
            try
            {
                // Expected size: 7 (player) + 18 (teammates) + 6 (monster) + 20 (nearby monsters) + 30 (collectibles) + 1 (temporal) = 82
                int expectedSize = 82;
        ...
        private bool TestTeammateDataEncoding()
        {
            try
            {
                float[] encodedState = stateEncoder.EncodeState(testGameState);

                // Teammate data should be encoded after player state (7 values)
                // Each teammate has 6 values: position(2) + velocity(2) + health(1) + downed(1)
                // First teammate starts at index 7
                int firstTeammateIndex = 7;

                // Check that teammate data is present and normalized
                bool hasTeammateData = false;
                for (int i = firstTeammateIndex; i < firstTeammateIndex + 18; i++) // 3 teammates * 6 values
                {
                    if (i < encodedState.Length)
                    {
                        float value = encodedState[i];
                        if (value < -1f || value > 1f)
                        {
                            Debug.LogError($"✗ Teammate data normalization failed at index {i}: value {value} outside [-1, 1]");
                            return false;
                        }
                        if (value != 0f) // Non-zero indicates data is present
                        {
                ...
                if (!hasTeammateData)
                {
                    Debug.LogWarning("⚠ Teammate data present but all values are zero");
                }

                if (logDetailedResults)
                    Debug.Log($"✓ Teammate data encoding test passed: {(hasTeammateData ? "data present" : "all zeros (valid)")}");
                return true;
            }
            catch (System.Exception e)
            {
                Debug.LogError($"✗ Teammate data encoding test failed: {e.Message}");
                return false;
            }
        }
