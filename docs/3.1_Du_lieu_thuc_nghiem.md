# 3.1. Dữ liệu thực nghiệm (Co-op Survivors RL)

Mục tiêu của chương này: mô tả rõ dữ liệu được sinh ra trong quá trình huấn luyện và suy luận multi-agent, cách thu thập, lưu trữ, cùng các chỉ số theo dõi để đánh giá mô hình RL trong game sinh tồn thể loại Survivors với chế độ Co-op.

## 3.1.1. Mục tiêu thu thập dữ liệu

- Đảm bảo mỗi phiên chạy (episode) có log đầy đủ: trạng thái đầu, chuỗi hành động (per-agent), reward theo thời gian, trạng thái kết thúc.
- Ghi nhận biến động hiệu năng (FPS, thời gian suy luận, băng thông sync) để kiểm soát chi phí tính toán và ảnh hưởng đến gameplay co-op.
